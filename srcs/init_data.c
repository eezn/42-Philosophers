...
/*   Updated: 2021/12/21 00:00:46 by jin-lee          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_arr_stick(t_data *data)
{
	...
	data->arr_stick = (t_chopstick *)malloc \
		(sizeof(t_chopstick) * data->num_philos);
	idx = -1;
	while (++idx < data->num_philos)
	{
		data->arr_stick[idx].chopstick_id = idx + 1;
		data->arr_stick[idx].mutex_lock = (pthread_mutex_t *)malloc \
...
void	init_data(int argc, char **argv, t_data *data)
{
	...
	data->program_status = ALIVE;
	init_arr_philo(data);
	init_arr_stick(data);
	data->mutex_pause = (pthread_mutex_t *)malloc(sizeof(pthread_mutex_t));
	pthread_mutex_init(data->mutex_pause, NULL);
}
